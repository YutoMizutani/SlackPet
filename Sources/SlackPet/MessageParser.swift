...
class MessageParser {
    ...
    typealias MessageHandler = ((_ message: String, _ date: Date?, _ channel: String) -> Bool)

    func append(_ handler: @escaping MessageHandler) {
        handlers.append(handler)
    }

    func parse(_ message: String, date: Date?, channel: String) {
        for handler in handlers {
            guard !handler(message, date, channel) else { break }
        }
    }
}
